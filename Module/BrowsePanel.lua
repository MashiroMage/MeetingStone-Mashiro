...
function BrowsePanel:OnInitialize()
        ...
        ActivityList:InitHeader {
            {
                ...
                iconHandler = function(activity)
                    ...
                    elseif activity:IsGoldLeader() then
                        return [[Interface\AddOns\MeetingStone\Media\GoldLeaderIcon]], 0.0, 1.0, 0, 1
    ...
    -- 申请/拒绝计数器（每次登陆重置）
    local appCount, rejCount = 0, 0

    local AppCounter = self:CreateFontString(nil, 'ARTWORK', 'GameFontHighlightRight')
    do
        AppCounter:SetPoint('BOTTOMRIGHT', MainPanel, -7, 7)
        AppCounter:SetText('|cFF00CC44已申请: 0|r  |cFFFF4444已拒绝: 0|r')
    end

    local function UpdateAppCounter()
        AppCounter:SetFormattedText('|cFF00CC44已申请: %d|r  |cFFFF4444已拒绝: %d|r', appCount, rejCount)
    end

    local appCounterFrame = CreateFrame('Frame')
    appCounterFrame:RegisterEvent('LFG_LIST_APPLICATION_STATUS_UPDATED')
    appCounterFrame:SetScript('OnEvent', function(_, _, resultid, status)
        if status == 'applied' then
            appCount = appCount + 1
            UpdateAppCounter()
        elseif status == 'declined' or status == 'timeout' then
            rejCount = rejCount + 1
            UpdateAppCounter()
        end
    end)
    ...
    do
        ...
        IconSummary:SetScript('OnEnter', function(button)
            ...
            GameTooltip:AddLine(format(
                ...
                L['申请中活动']), 1, 1, 1)
            local title = format(
                [[|TInterface\AddOns\MeetingStone\Media\GoldLeaderIcon:20:20:0:0:128:128:0:128:0:128|t %s]],
                L['金牌导师'])
